#!/usr/bin/env python3
"""
执行 046 迁移：scrape_runs 表 + admin_get_scrape_runs RPC
用于管理员在 Admin 页面查看抓取运行记录（开始时间、耗时、操作人、结果）
用法: python3 run-migration-046.py 或 SUPABASE_DB_PASSWORD=密码 python3 run-migration-046.py
"""
import os
import sys
...
def main():
    ...
    script_dir = Path(__file__).resolve().parent
    sql_path = script_dir / 'supabase' / 'migrations' / '046_scrape_runs.sql'
    with open(sql_path, 'r', encoding='utf-8') as f:
        sql = f.read()
    print('正在执行 046 迁移（scrape_runs 表 + admin_get_scrape_runs RPC）...')
