#!/usr/bin/env python3
"""
执行 020 迁移：customer_groups 添加 is_active 列（客户 inactive 打标）
用法: python3 run-migration-020.py 或 SUPABASE_DB_PASSWORD=密码 python3 run-migration-020.py
"""
import os
import sys
...
def main():
    ...
    script_dir = Path(__file__).resolve().parent
    sql_path = script_dir / 'supabase' / 'migrations' / '020_customer_group_is_active.sql'
    with open(sql_path, 'r', encoding='utf-8') as f:
        sql = f.read()
    print('正在执行 020 迁移（customer_groups.is_active）...')
