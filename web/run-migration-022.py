#!/usr/bin/env python3
"""
执行 022 迁移：properties 添加 price_value、price_description 列
用法: python3 run-migration-022.py 或 SUPABASE_DB_PASSWORD=密码 python3 run-migration-022.py
"""
import os
import sys
...
def main():
    ...
    script_dir = Path(__file__).resolve().parent
    sql_path = script_dir / 'supabase' / 'migrations' / '022_price_value_description.sql'
    with open(sql_path, 'r', encoding='utf-8') as f:
        sql = f.read()
    print('正在执行 022 迁移（properties.price_value / price_description）...')
