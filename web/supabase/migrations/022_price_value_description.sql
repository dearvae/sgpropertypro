-- 将价格拆分为 price_value（数值）和 price_description（描述，如 negotiable、starting from）
-- 展示时组合为 "price_value price_description"，中间有空距
...
declare
  ...
  v_lower text;
begin
  for r in select id, price from public.properties where price is not null and trim(price) != ''
  loop
    v_lower := lower(r.price);
    v_desc := null;
    v_val := trim(r.price);
    if v_lower ~ '\bnegotiable\b' then
      v_desc := 'negotiable';
      v_val := trim(regexp_replace(r.price, '\s*negotiable\s*', ' ', 'gi'));
    elsif v_lower ~ 'starting\s+from\s+' then
      v_desc := 'Starting from';
      v_val := trim(regexp_replace(r.price, '^\s*starting\s+from\s+', '', 'gi'));
    elsif v_lower ~ '\b(poa|price on request|call for price|contact for price)\b' then
      v_desc := trim((regexp_matches(v_lower, '(?:poa|price on request|call for price|contact for price)', 'i'))[1]);
      v_val := trim(regexp_replace(r.price, '(?:poa|price on request|call for price|contact for price)\s*', '', 'gi'));
    end if;
    update public.properties set price_value = nullif(trim(v_val), ''), price_description = v_desc where id = r.id;
  end loop;
end $$;

-- 更新 get_client_view：property 中返回 price_value、price_description
