#!/usr/bin/env python3
"""
执行 037 迁移：agent 自售/出租房源（customer_groups 添加 group_type、property_id）
用法: python3 run-migration-037.py 或 SUPABASE_DB_PASSWORD=密码 python3 run-migration-037.py
"""
import os
import sys
...
def main():
    ...
    script_dir = Path(__file__).resolve().parent
    sql_path = script_dir / 'supabase' / 'migrations' / '037_agent_listings.sql'
    with open(sql_path, 'r', encoding='utf-8') as f:
        sql = f.read()
    print('正在执行 037 迁移（customer_groups.group_type、property_id）...')
