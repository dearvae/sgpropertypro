#!/usr/bin/env python3
"""
执行 041 迁移：将 pending_appointments 加入 Realtime
用法: python3 run-migration-041.py 或 SUPABASE_DB_PASSWORD=密码 python3 run-migration-041.py
"""
import os
import sys
...
def main():
    ...
    script_dir = Path(__file__).resolve().parent
    sql_path = script_dir / 'supabase' / 'migrations' / '041_realtime_pending_appointments.sql'
    with open(sql_path, 'r', encoding='utf-8') as f:
        sql = f.read()
    print('正在执行 041 迁移（pending_appointments Realtime）...')
