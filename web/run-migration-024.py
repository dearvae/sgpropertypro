#!/usr/bin/env python3
"""
执行 024 迁移：properties 添加 last_scraped_at 列（1 小时冷却限流）
用法: python3 run-migration-024.py 或 SUPABASE_DB_PASSWORD=密码 python3 run-migration-024.py
"""
import os
import sys
...
def main():
    ...
    script_dir = Path(__file__).resolve().parent
    sql_path = script_dir / 'supabase' / 'migrations' / '024_property_last_scraped_at.sql'
    with open(sql_path, 'r', encoding='utf-8') as f:
        sql = f.read()
    print('正在执行 024 迁移（properties.last_scraped_at）...')
